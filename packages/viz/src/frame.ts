...
export function compose(spec:FigureSpec,locale:Locale,mobile=false){
  ...
  let y=mobile?38:52,body='';
  if(spec.eyebrow){
    const eyebrowSize=mobile?15:18;
    body+=svgText(pad,y-(mobile?6:8),tr(spec.eyebrow,locale),eyebrowSize,C.blue,600,width-2*pad);
    y+=eyebrowSize+(mobile?14:20);
  }
  body+=svgText(pad,y,title,titleSize,C.ink,600,width-2*pad);
