...
def frame(fig,step,title,subtitle,body,height,label,alt):
    slug=f'pp-{fig:02}-step-{step}'
...
for LANG in ['ko','en']:
    ...
    for step,now in enumerate(times):
        ...
        frame(3,step,('배치를 나누면 다음 GPU가 먼저 시작할 수 있습니다','Splitting the batch lets the next GPU start sooner'),('다음을 눌러 같은 시간축에서 두 실행의 진행과 완료를 비교하세요.','Use Next to compare progress and completion on the same time axis.'),b,1410,label,(f'시각 {now}. 통째 배치는 각 단계 4칸씩 총 16칸, 마이크로배치 파이프라인은 총 7칸이다. 같은 네 입력과 네 GPU의 순전파를 비교한다.',f'Time {now}. The whole batch takes four slots per stage, finishing in 16; microbatch pipelining finishes in seven. Compare forward passes with the same four inputs and four GPUs.'))
