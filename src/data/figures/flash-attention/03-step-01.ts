...
export default {
  articleId:"flash-attention",figureId:"03-step-01",number:"03-step-01",
  ...
  alt:["왼쪽 Q 두 행, 위쪽 K 네 벡터, 아래쪽 V 네 벡터, 오른쪽 출력 누적 벡터를 배치한다. K와 V는 토큰을 가로로 나열한 전치 배치다. 중앙에는 현재 2×2 타일의 점수 또는 정규화 전 exp 값만 표시한다. Q와 출력 위치를 고정하고 두 K/V 타일을 순회한다. 중앙의 전체 격자는 처리 위치 지도이며 전체 점수 행렬을 저장하지 않는다. 스케일링과 마스크를 생략한 교육용 계산이다. 현재 단계: 입력과 출력의 위치","Two Q rows on the left, four K vectors above, four V vectors below, and output accumulator vectors on the right. K and V are transposed to arrange tokens horizontally. The center shows only the current 2×2 tile of scores or unnormalized exponentials. Q and output positions stay fixed while traversing two K/V tiles. The full grid maps processing positions and does not store the full score matrix. This educational example omits scaling and masking. Current step: Input and output positions."],
  ...
  panels(locale:Locale){
    ...
    p.el('text',{x:76,y:233,fill:C.blue,"font-size":27,"font-weight":700,"text-anchor":"start"},["1 / 9    입력과 출력의 위치","1 / 9    Input and output positions"]);
    p.el('text',{x:76,y:269,fill:C.ink,"font-size":22,"font-weight":400,"text-anchor":"start"},["Q와 출력 위치는 고정하고, 같은 토큰의 K와 V를 함께 읽습니다.","Keep Q and output positions fixed. Read K and V for the same tokens together."]);
    ...
    p.el('text',{x:550,y:549,fill:C.teal,"font-size":24,"font-weight":700,"text-anchor":"middle"},["계산할 위치","Computation positions"]);
    ...
    p.el('text',{x:77,y:1098,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},["왼쪽 Q 한 행 ↔ 중앙 한 행 ↔ 오른쪽 출력 한 행","One Q row on the left ↔ one central row ↔ one output row on the right"]);
    p.el('text',{x:77,y:1145,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},["위아래의 Kⱼ와 Vⱼ는 같은 토큰입니다. 두 벡터를 한 쌍으로 읽습니다.","Kⱼ above and Vⱼ below belong to the same token. Read them as a pair."]);
