...
export default {
  articleId:"context-parallelism",figureId:"cp-04-step-3",number:"cp-04-step-3",
  ...
  alt:["전체 토큰, 일부 헤드의 출력 O를 원래 토큰 소유자에게 보낸다.","Send attention output O, partitioned by head, back to the original token owners."],
  ...
  panels(locale:Locale){
    ...
    p.el("rect",{x:739,y:137,width:220,height:44,rx:10,fill:C.purpleFill,stroke:C.purpleFill,"stroke-width":1.5});
    p.el('text',{x:849,y:166,"font-size":20,fill:C.purple,"font-weight":700,"text-anchor":"middle"},["역 All-to-All","Reverse All-to-All"]);
    ...
    p.el('text',{x:312,y:753,"font-size":21,fill:C.blue,"font-weight":700,"text-anchor":"middle"},["내 헤드의 출력을 다시 토큰별로 전송","Send my head’s outputs back by token"]);
    ...
    p.el('text',{x:888,y:753,"font-size":21,fill:C.orange,"font-weight":700,"text-anchor":"middle"},["내 헤드의 출력을 다시 토큰별로 전송","Send my head’s outputs back by token"]);
    p.el("path",{d:"M365,637 L834,637",fill:"none",stroke:C.purple,"stroke-width":2,"marker-end":"url(#ported-arrow)"});
    p.el('text',{x:600,y:621,"font-size":20,fill:C.purple,"font-weight":700,"text-anchor":"middle"},["t2·t3의 h0 출력 → GPU 1","h0 outputs for t2·t3 → GPU 1"]);
    p.el("path",{d:"M834,701 L365,701",fill:"none",stroke:C.purple,"stroke-width":2,"marker-end":"url(#ported-arrow)"});
    p.el('text',{x:600,y:683,"font-size":20,fill:C.purple,"font-weight":700,"text-anchor":"middle"},["t0·t1의 h1 출력 → GPU 0","h1 outputs for t0·t1 → GPU 0"]);
    p.el("rect",{x:166,y:864,width:868,height:44,rx:10,fill:C.purpleFill,stroke:C.purpleFill,"stroke-width":1.5});
    p.el('text',{x:600,y:893,"font-size":20,fill:C.purple,"font-weight":700,"text-anchor":"middle"},["역 All-to-All: t0·t1 출력은 GPU 0으로, t2·t3 출력은 GPU 1로","Reverse All-to-All: t0·t1 outputs → GPU 0, t2·t3 outputs → GPU 1"]);
    p.el('text',{x:600,y:949,"font-size":22,fill:C.ink,"font-weight":700,"text-anchor":"middle"},["Q·K·V를 되돌리는 대신, 계산된 어텐션 출력 O를 재배치합니다.","Rearrange computed attention output O, rather than sending Q·K·V back."]);
