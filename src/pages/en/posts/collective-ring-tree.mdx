...
The figure first shows the four GPUs’ inputs, then follows three transfers from top to bottom. GPU positions and colors stay fixed. Arrows identify the chunk being sent, while GPU boxes show the sum computed after receipt.

[![Four GPUs arranged in a ring exchange and reduce chunks in three steps. Arrows show sent values and boxes show sums after receipt.](/images/collective-ring-tree/en/01-ring-reduce-scatter.png)](/images/collective-ring-tree/en/01-ring-reduce-scatter.png)
...
[![Four GPUs arranged in a ring each forward one chunk per step. After three steps, all hold A through D.](/images/collective-ring-tree/en/02-ring-all-gather.png)](/images/collective-ring-tree/en/02-ring-all-gather.png)
...
A Tree defines parent–child relationships between participants. In a simple example, GPU 1 first sends to 0, and GPU 3 to 2. These two reductions proceed together, then GPU 2 sends its partial sum to GPU 0. Finally, the complete sum travels back along reversed edges. GPU positions stay fixed across the four panels below. Dark arrows show the transfers performed in that step, and GPU boxes show values after the transfers.

[![Four panels keep the same tree layout: two reduction steps to GPU 0, followed by two broadcast steps along reversed edges.](/images/collective-ring-tree/en/03-tree.png)](/images/collective-ring-tree/en/03-tree.png)
