...
export default {
  articleId:"flash-attention",figureId:"03-step-05",number:"03-step-05",
  ...
  alt:["왼쪽 Q 두 행, 위쪽 K 네 벡터, 아래쪽 V 네 벡터, 오른쪽 출력 누적 벡터를 배치한다. K와 V는 토큰을 가로로 나열한 전치 배치다. 중앙에는 현재 2×2 타일의 점수 또는 정규화 전 exp 값만 표시한다. Q와 출력 위치를 고정하고 두 K/V 타일을 순회한다. 중앙의 전체 격자는 처리 위치 지도이며 전체 점수 행렬을 저장하지 않는다. 스케일링과 마스크를 생략한 교육용 계산이다. 현재 단계: 다음 타일 · 새 점수 계산","Two Q rows on the left, four K vectors above, four V vectors below, and output accumulator vectors on the right. K and V are transposed to arrange tokens horizontally. The center shows only the current 2×2 tile of scores or unnormalized exponentials. Q and output positions stay fixed while traversing two K/V tiles. The full grid maps processing positions and does not store the full score matrix. This educational example omits scaling and masking. Current step: Next tile · Compute new scores."],
  ...
  panels(locale:Locale){
    ...
    p.el('text',{x:76,y:233,fill:C.blue,"font-size":27,"font-weight":700,"text-anchor":"start"},["5 / 9    다음 타일 · 새 점수 계산","5 / 9    Next tile · Compute new scores"]);
    p.el('text',{x:76,y:269,fill:C.ink,"font-size":22,"font-weight":400,"text-anchor":"start"},["Q와 a는 그대로 두고 K₂·K₃, V₂·V₃으로 이동합니다.","Keep Q and a in place. Move to K₂, K₃ and V₂, V₃."]);
    ...
    p.el("path",{d:"M610,480 L610,550",fill:"none",stroke:C.purple,"stroke-width":3,"marker-end":markerUrl(C.purple)});
    ...
    p.el("path",{d:"M730,480 L730,550",fill:"none",stroke:C.purple,"stroke-width":3,"marker-end":markerUrl(C.purple)});
    ...
    p.el("rect",{x:551,y:573,width:114,height:73,rx:12,fill:C.orangeFill,stroke:C.line,"stroke-width":1.5});
    p.el('text',{x:608,y:617.5,fill:C.orange,"font-size":27,"font-weight":600,"text-anchor":"middle"},"3");
    p.el("rect",{x:671,y:573,width:114,height:73,rx:12,fill:C.orangeFill,stroke:C.line,"stroke-width":1.5});
    p.el('text',{x:728,y:617.5,fill:C.orange,"font-size":27,"font-weight":600,"text-anchor":"middle"},"2");
    ...
    p.el("rect",{x:551,y:677,width:114,height:73,rx:12,fill:C.orangeFill,stroke:C.line,"stroke-width":1.5});
    p.el('text',{x:608,y:721.5,fill:C.orange,"font-size":27,"font-weight":600,"text-anchor":"middle"},"0");
    p.el("rect",{x:671,y:677,width:114,height:73,rx:12,fill:C.orangeFill,stroke:C.line,"stroke-width":1.5});
    p.el('text',{x:728,y:721.5,fill:C.orange,"font-size":27,"font-weight":600,"text-anchor":"middle"},"1");
    ...
    p.el('text',{x:77,y:1098,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},["첫 타일의 점수·exp 값은 소비하고, m·ℓ·a를 유지합니다.","Consume the first tile’s scores and exponentials; keep m, ℓ, and a."]);
    p.el('text',{x:77,y:1145,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},["같은 Q로 다음 K 타일의 점수를 계산합니다. 아래 V도 함께 이동합니다.","Compute scores for the next K tile with the same Q. V below moves with K."]);
