...
export default {
  articleId:"flash-attention",figureId:"03-step-08",number:"03-step-08",
  ...
  alt:["왼쪽 Q 두 행, 위쪽 K 네 벡터, 아래쪽 V 네 벡터, 오른쪽 출력 누적 벡터를 배치한다. K와 V는 토큰을 가로로 나열한 전치 배치다. 중앙에는 현재 2×2 타일의 점수 또는 정규화 전 exp 값만 표시한다. Q와 출력 위치를 고정하고 두 K/V 타일을 순회한다. 중앙의 전체 격자는 처리 위치 지도이며 전체 점수 행렬을 저장하지 않는다. 스케일링과 마스크를 생략한 교육용 계산이다. 현재 단계: 다음 타일 · 같은 출력에 누적","Two Q rows on the left, four K vectors above, four V vectors below, and output accumulator vectors on the right. K and V are transposed to arrange tokens horizontally. The center shows only the current 2×2 tile of scores or unnormalized exponentials. Q and output positions stay fixed while traversing two K/V tiles. The full grid maps processing positions and does not store the full score matrix. This educational example omits scaling and masking. Current step: Next tile · Accumulate into the same output."],
  ...
  panels(locale:Locale){
    ...
    p.el('text',{x:76,y:233,fill:C.blue,"font-size":27,"font-weight":700,"text-anchor":"start"},["8 / 9    다음 타일 · 같은 출력에 누적","8 / 9    Next tile · Accumulate into the same output"]);
    p.el('text',{x:76,y:269,fill:C.ink,"font-size":22,"font-weight":400,"text-anchor":"start"},["새 Value 가중합을 보정한 a에 더합니다. 이전 점수는 다시 읽지 않습니다.","Add the new Value weighted sum to rescaled a without rereading previous scores."]);
    ...
    p.el('text',{x:985.5,y:617,fill:C.teal,"font-size":22,"font-weight":600,"text-anchor":"middle"},"0.639");
    p.el("rect",{x:1034,y:582,width:91,height:54,rx:12,fill:C.tealFill,stroke:C.line,"stroke-width":1.5});
    p.el('text',{x:1079.5,y:617,fill:C.teal,"font-size":22,"font-weight":600,"text-anchor":"middle"},"3.104");
    ...
    p.el('text',{x:985.5,y:721,fill:C.teal,"font-size":22,"font-weight":600,"text-anchor":"middle"},"2.368");
    p.el("rect",{x:1034,y:686,width:91,height:54,rx:12,fill:C.tealFill,stroke:C.line,"stroke-width":1.5});
    p.el('text',{x:1079.5,y:721,fill:C.teal,"font-size":22,"font-weight":600,"text-anchor":"middle"},"1.374");
    ...
    p.el("path",{d:"M610,851 L610,772",fill:"none",stroke:C.teal,"stroke-width":3,"marker-end":markerUrl(C.teal)});
    ...
    p.el("path",{d:"M730,851 L730,772",fill:"none",stroke:C.teal,"stroke-width":3,"marker-end":markerUrl(C.teal)});
    ...
    p.el('text',{x:77,y:1098,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},"Q₀: [0.271, 0.736] + 1 × [0, 2] + 0.368 × [1, 1]");
    p.el('text',{x:77,y:1145,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},"Q₁: [2, 0.736] + 0.135 × [0, 2] + 0.368 × [1, 1]");
