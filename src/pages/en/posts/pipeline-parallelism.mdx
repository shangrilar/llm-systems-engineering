...
Partitioning layers does not immediately make every GPU compute at once. Later GPUs must wait for results from earlier GPUs. This article first examines layer placement and the directions of forward and backward passes. We then see what it means to split a batch into smaller groups, and compare whole-batch execution with microbatch execution to follow how GPUs work together. Finally, we examine the remaining waits and the effect of partition size.
...
Either direction requires the results of preceding computations. In a forward pass for one input, GPU 1 cannot begin before GPU 0 has produced the activations. Multiple GPUs do not compute all layers of the same input simultaneously. Reducing this wait requires overlapping computation for different inputs.
