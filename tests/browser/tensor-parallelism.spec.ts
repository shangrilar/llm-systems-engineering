...
import steps from '../../src/data/tp-steps.json' with { type: 'json' };

for (const locale of ['ko', 'en'] as const) {
  test(`${locale}: TP controls navigate independently through every state`, async ({ page }) => {
    const base = `/images/tensor-parallelism/${locale === 'en' ? 'en/' : ''}`;
    await page.goto(`${locale === 'en' ? '/en' : ''}/posts/tensor-parallelism/`);
    const players = page.locator('[data-tp-steps]');
    await expect(players).toHaveCount(5);
    for (const width of [360, 1280]) {
      await page.setViewportSize({ width, height: 960 });
      for (const figure of ['01', '02', '03', '04', '05'] as const) {
  ...
  test(`${locale}: TP retain the completed figure without JavaScript`, async ({ browser }) => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const completed = players.nth(i).locator('.step-info a');
      expect((await page.request.get((await completed.getAttribute('href'))!)).status()).toBe(200);
    }
    await page.close();
  });
}
