...
const slug = 'attention-and-mlp';
for (const locale of ['ko', 'en']) {
  test(`${locale}: attention and MLP article, five figures, full-size images and language switch`, async ({ page }) => {
    const errors: string[] = [];
    ...
    for (const width of [360, 768, 1280]) {
      ...
      await mkdir('test-results/attention-mlp-article', { recursive: true });
      await page.screenshot({ path: `test-results/attention-mlp-article/${locale}-${width}.png`, fullPage: true });
    }
