...
export default {
  articleId:"flash-attention",figureId:"03-step-06",number:"03-step-06",
  ...
  alt:["왼쪽 Q 두 행, 위쪽 K 네 벡터, 아래쪽 V 네 벡터, 오른쪽 출력 누적 벡터를 배치한다. K와 V는 토큰을 가로로 나열한 전치 배치다. 중앙에는 현재 2×2 타일의 점수 또는 정규화 전 exp 값만 표시한다. Q와 출력 위치를 고정하고 두 K/V 타일을 순회한다. 중앙의 전체 격자는 처리 위치 지도이며 전체 점수 행렬을 저장하지 않는다. 스케일링과 마스크를 생략한 교육용 계산이다. 현재 단계: 다음 타일 · 이전 누적값 보정","Two Q rows on the left, four K vectors above, four V vectors below, and output accumulator vectors on the right. K and V are transposed to arrange tokens horizontally. The center shows only the current 2×2 tile of scores or unnormalized exponentials. Q and output positions stay fixed while traversing two K/V tiles. The full grid maps processing positions and does not store the full score matrix. This educational example omits scaling and masking. Current step: Next tile · Rescale previous accumulators."],
  ...
  panels(locale:Locale){
    ...
    p.el('text',{x:76,y:233,fill:C.blue,"font-size":27,"font-weight":700,"text-anchor":"start"},["6 / 9    다음 타일 · 이전 누적값 보정","6 / 9    Next tile · Rescale previous accumulators"]);
    p.el('text',{x:76,y:269,fill:C.ink,"font-size":22,"font-weight":400,"text-anchor":"start"},["Q₀의 최댓값만 커집니다. 해당 행의 ℓ과 a를 같은 배율로 보정합니다.","Only Q₀’s maximum rises. Rescale its ℓ and a by the same factor."]);
    ...
    p.el('text',{x:1033,y:671,fill:C.purple,"font-size":19,"font-weight":600,"text-anchor":"middle"},"m = 3 · ℓ ≈ 0.503");
    ...
    p.el('text',{x:77,y:1098,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},["Q₀: m 2 → 3 · 기존 ℓ과 a에 exp(−1) ≈ 0.368을 곱함","Q₀: m 2 → 3 · Multiply previous ℓ and a by exp(−1) ≈ 0.368"]);
    p.el('text',{x:77,y:1145,fill:C.ink,"font-size":25,"font-weight":400,"text-anchor":"start"},["Q₁: m = 2 유지 · 보정 배율 1이므로 기존 ℓ과 a를 유지","Q₁: Keep m = 2 · Factor 1 leaves previous ℓ and a unchanged"]);
