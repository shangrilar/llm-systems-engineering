...
export default {
  articleId:"collective-communication-basics",figureId:"basics-04-gather",number:"basics-04-gather",
  layout:'wide',screens:['desktop'],
  eyebrow:["그림 4","Figure 4"],
  title:["Gather: 각자의 조각을 한곳으로","Gather: collect chunks at one GPU"],
  subtitle:["각 GPU의 배열을 rank 순서로 이어 붙여 root에 모읍니다.","Concatenate each GPU’s array in rank order at the root."],
  alt:["각 GPU의 입력은 [1,2], [10,20], [100,200], [1000,2000]이다. Gather 완료 후 root인 GPU 0의 출력은 이 배열들을 rank 순서로 이어 붙인 [1,2,10,20,100,200,1000,2000]이다. 배열의 네 행은 원래 각 GPU의 두 원소짜리 조각이며 하나의 일차원 배열을 접어 표시했다. 다른 GPU는 출력 대상이 아니며 입력은 유지된다.","Each GPU supplies two elements. Gather concatenates [1,2], [10,20], [100,200], [1000,2000] in rank order at GPU 0."],
  caption:["결과는 도착 순서가 아니라 rank 순서로 이어 붙입니다. 다른 GPU의 입력은 유지됩니다.","Results are concatenated in rank order, not arrival order. Other GPUs keep their inputs."],
  sources:[],
  panels(locale:Locale){
    ...
    p.el('text',{x:600,y:463,fill:C.ink,"font-size":25,"font-weight":600,"text-anchor":"middle"},["각 GPU의 입력 2개 원소 → GPU 0의 출력 8개 원소","Each GPU: 2 input elements → GPU 0: 8 output elements"]);
    ...
    p.el('text',{x:460,y:771,fill:C.muted,"font-size":23,"font-weight":400,"text-anchor":"middle"},["출력 대상 아님","No result here"]);
    ...
    p.el('text',{x:740,y:771,fill:C.muted,"font-size":23,"font-weight":400,"text-anchor":"middle"},["출력 대상 아님","No result here"]);
    ...
    p.el('text',{x:1020,y:771,fill:C.muted,"font-size":23,"font-weight":400,"text-anchor":"middle"},["출력 대상 아님","No result here"]);
    return [p];
  },
