...
| Shared memory | Up to 228 KiB per SM | About 38.5 TB/s aggregate (structural estimate; conditions below) |
...
As an example of the bandwidth scale, [Figure 1 and Section 2.1 of the FlashAttention paper](https://arxiv.org/pdf/2205.14135#page=2) give about 19 TB/s for A100 on-chip SRAM and about 1.5 TB/s for HBM. This is **a bandwidth ratio of about 13 to 1**, and the paper explicitly describes the SRAM figure as an estimate. These figures describe A100, so we do not apply the same ratio to B300.

We can also estimate the scale of B300 shared-memory bandwidth from its structure. The [CUDA guide](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/index.html#shared-memory-and-memory-banks) describes 32 shared-memory banks, each providing 4 bytes per clock cycle. A [public device-query result for B300 SXM6 AC](https://browser.geekbench.com/v6/compute/5203754) reports 148 SMs and a maximum clock of 2,032 MHz. Assuming this clock is sustained and all SMs use their 32 banks simultaneously, **32 × 4 bytes × 2.032 GHz × 148 ≈ 38.5 TB/s**. This is about 260 GB/s per SM, with an aggregate of **about 4.8 times** B300's peak HBM bandwidth of 8 TB/s.

The shared-memory bandwidth in the table is an estimate calculated under these conditions, not an official manufacturer bandwidth specification or a result we measured. It sums the throughput of all SMs accessing their own shared memory concurrently; one SM cannot use 38.5 TB/s. Actual throughput depends on operating clocks, the number of participating SMs, instructions, and access patterns. **The bandwidth ratio does not directly translate into the same program speedup.**

This comparison illustrates how much data a small storage space close to the compute units can supply. Keeping inputs in shared memory reduces repeated global-memory accesses, and keeping them in registers can reduce repeated shared-memory accesses as well. We have not confirmed a comparable register bandwidth figure, so it is not listed in the table.
