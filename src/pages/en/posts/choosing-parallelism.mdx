...
This article follows **making room to run → replicating the working group → adjusting partitioning and replication**. We use inference with a Dense model that processes independent requests using fixed weights. We connect the methods introduced earlier, while keeping the figures focused on TP and DP.
...
These numbers illustrate a capacity relationship. In practice, not all weights may partition evenly, and execution memory changes with input length, the number of requests processed together, and TP size. Peak memory use and headroom therefore need to be checked while running the target workload. [vLLM's parallelism guidance](https://docs.vllm.ai/en/latest/serving/parallelism_scaling/) likewise describes choosing GPUs to hold the model, then checking memory and concurrency capacity for processing requests.
...
The two groups, however, process different requests. This is **TP 2 × DP 2**. TP 2 means that two GPUs jointly process one partitioned model replica, and DP 2 means running two copies of that configuration. **The unit replicated by DP may be a single GPU or a model replica spanning multiple GPUs.** [vLLM's DP documentation](https://docs.vllm.ai/en/stable/serving/data_parallel_deployment/) also describes running multiple TP-partitioned replicas to process independent request batches.

More replicas create room to process more requests in the same amount of time. The number of GPUs participating in one request stays the same, while more groups become available for other requests. Throughput can increase when enough requests arrive and work is distributed evenly across the groups. With too few requests or work concentrated in one group, gains need not match the replica count.

This example uses Dense inference with fixed weights. The two groups do not need to combine model intermediate results with each other while computing their requests; each group maintains the state of its own requests. A through D illustrate assignment, not a limit of two requests per group.
...
Which GPUs form each group also matters. If links within GPUs 0–1 and within GPUs 2–3 are fast, but the connection between those pairs is slower, keeping TP's repeated communication inside each pair may be beneficial. The figure's arrows are not a cable diagram, so distinguish [logical relationships from physical connections](/en/posts/collective-ring-tree/).

If activations or a long context consume much of the memory during execution, methods introduced earlier, such as [SP](/en/posts/sequence-parallelism/) or [CP](/en/posts/context-parallelism/), can be considered alongside the initial configuration. A model partition chosen first can still be revised based on the target inputs and measured results.
