...
Recall the execution model from [How the CPU and GPU Execute Work Together](https://llm-systems-engineering.pages.dev/en/posts/cpu-gpu-work-execution/). CPU code requests GPU work, and the GPU executes the submitted operations. Operations submitted to the same stream proceed in order, and **the CPU call finishes at a different point from the GPU operation**. We will now connect this execution flow across two GPUs.
...
Even after separating the streams, we must preserve **the requirement that B execute after the receive completes**. We use the event mechanism introduced earlier. Process 1's CPU code first submits the recording of event E after the receive in the communication stream. It then submits C, a wait for event E to complete, and B in the compute stream, in that order.
...
If the CPU needs to read the actual values of y, one more step is required. Even after B finishes, y is still in GPU memory. As discussed earlier, we must copy the result to CPU memory and ensure that copy completes. Waiting for completion and moving data are separate operations.
