...
## Weigh partitioned weights against added communication

EP reduces the need for each GPU to store every expert's weights. Here, each GPU holds two of four experts. But replicated attention, router, and other common weights remain, and space is also needed for dispatched inputs and returned outputs. Halving the number of experts per GPU does not halve total GPU memory usage.

Training and inference both follow routing, dispatch, expert computation, return, and combination in the forward pass. Training adds backward gradient transfers and weight updates. The figures show forward passes; they omit training gradient synchronization for DP replicas and backward scheduling.

The key idea is **that different model parts can use different parallelism strategies**. Common layers apply the same weights to different inputs, while expert layers partition weights and exchange the tokens that need them. [How Should We Arrange Multiple GPUs?](/en/posts/choosing-parallelism/) next examines how to choose and combine these strategies.
