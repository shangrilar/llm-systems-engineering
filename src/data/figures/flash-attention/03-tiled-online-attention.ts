...
export default {
  articleId:"flash-attention",figureId:"03-tiled-online-attention",number:"03-tiled-online-attention",
  ...
  alt:["왼쪽 Q 두 행, 위쪽 K 네 벡터, 아래쪽 V 네 벡터, 오른쪽 출력 누적 벡터를 배치한다. K와 V는 토큰을 가로로 나열한 전치 배치다. 중앙에는 현재 2×2 타일의 점수 또는 정규화 전 exp 값만 표시한다. Q와 출력 위치를 고정하고 두 K/V 타일을 순회한다. 중앙의 전체 격자는 처리 위치 지도이며 전체 점수 행렬을 저장하지 않는다. 스케일링과 마스크를 생략한 교육용 계산이다.","Two Q rows on the left, four K vectors above, four V vectors below, and output accumulator vectors on the right. K and V are transposed to arrange tokens horizontally. The center shows only the current 2×2 tile of scores or unnormalized exponentials. Q and output positions stay fixed while traversing two K/V tiles. The full grid maps processing positions and does not store the full score matrix. This educational example omits scaling and masking."],
