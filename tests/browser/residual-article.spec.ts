...
for (const locale of ['ko', 'en']) {
  test(`${locale}: residual article, four figures, full-size images and language switch`, async ({ page }) => {
    ...
    await expect(page.getByRole('navigation', { name: locale === 'ko' ? '이전과 다음 글' : 'Previous and next articles' }).getByRole('link').first()).toHaveAttribute('href', `${prefix}/posts/embedding-to-lm-head/`);
